...
const Signup: React.FC = (): JSX.Element => {
		...
		try {
			...
			if (axios.isAxiosError(error)) {
				if (error.response?.status === StatusCodes.BadRequest) {
					const message =
						JSON.parse(error.response?.data.message).pop().message ||
						"Bad Request!";
					setError(message);
	...
	return (
		<section className="min-h-dvh w-full flex items-center justify-center">
			<div className="flex flex-col gap-8 justify-center py-10 px-16 border border-input rounded">
				...
				>
					...
					<Button variant="default" type="submit" disabled={isLoading}>
						{!isLoading ? "Sign up" : <ClipLoader size={20} color="#0F172A" />}
					</Button>
				</form>
